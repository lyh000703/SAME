...
@WebServlet("/manager/menteeList")
public class MenteeListController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            // 멘티 목록 조회
            List<Member> list = managerService.selectMenteeList(pi, sort);
            
            request.setAttribute("pi", pi);
            request.setAttribute("list", list);
            request.setAttribute("sort", sort);
            
            request.getRequestDispatcher("/views/manager/menteeInfoList.jsp").forward(request, response);
